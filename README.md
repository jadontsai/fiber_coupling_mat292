...
To run the files, all files should be downloaded. First the adaptive gradient descent function should be called to a satisfactory tolerance by running adaptive_gradient_descent.py (decision of exact tolerance up to you; if you wanted to use our values you can just call the functions as is as they are the ones currently coded in as parameters and the same as the report ones). Once a rough location is reached, nedler mead should be run to find a more precise location of maxima by running power_function.py. Parameters are denoted by comments and can be freely changed following requirements mentioned in comments.
